...
const renderText = (text: string | undefined, lightTheme = true) => {
  if (!text) return <p className="text-slate-400 text-sm italic">데이터를 분석 중입니다...</p>;
...
const SectionHeader = ({ num, title, dark = false }: { num: string; title: string; dark?: boolean }) => (
  <div className={`w-full mb-10 pb-5 border-b flex items-end gap-5 ${dark ? 'border-slate-800' : 'border-slate-200'}`}>
    ...
    <div className={`px-4 py-1.5 rounded-full border-2 text-[10px] font-black italic tracking-widest ${dark ? 'border-amber-500/20 text-amber-500/60' : 'border-slate-100 text-slate-300'}`}>VIP 프리미엄 보고서</div>
  </div>
);
...
export const DeepReportPDFTemplate: React.FC<DeepReportPDFTemplateProps> = ({ sajuData, parsedContent, clientName }) => {
  ...
  // Dynamic total pages based on roadmap length
  const roadmapLength = parsedContent.threeYearRoadmap?.length || 0;
  const TOTAL_PAGES = 5 + roadmapLength; 
  ...
  return (
    <div id="pdf-capture-zone" className="font-sans text-slate-900 bg-white" style={{ width: '210mm' }}>
      ...
      <PageWrapper pageNumber={3} totalPages={TOTAL_PAGES}>
        <SectionHeader num="02" title={parsedContent.wealthAndCareer?.title || "재물 그릇의 크기와 사회적 성취"} />
      ...
      {parsedContent.threeYearRoadmap?.details?.map((yearData: any, idx: number) => (
        <PageWrapper key={yearData.year} pageNumber={5 + idx} totalPages={TOTAL_PAGES} bg={idx === 0 ? "bg-slate-900" : "bg-white"} extraClass={idx === 0 ? "text-white" : ""}>
          <div className={`w-full mb-8 pb-4 border-b flex justify-between items-end ${idx === 0 ? 'border-slate-700' : 'border-slate-200'}`}>
             <h2 className="text-4xl font-black tracking-tighter italic">{idx === 0 ? (parsedContent.threeYearRoadmap?.title || `${yearData.year}년 미래 로드맵`) : `${yearData.year}년 미래 로드맵`}</h2>
      ...
      {/* Final Action Plan */}
      <PageWrapper pageNumber={TOTAL_PAGES} totalPages={TOTAL_PAGES} bg="bg-slate-950" extraClass="text-white">
        <SectionHeader num="05" title={parsedContent.actionPlan?.title || "운명을 바꾸는 마스터의 마스터플랜"} dark />
        ...
        <div className="flex flex-col gap-10 flex-1">
          ...
          <div className="flex flex-col items-center mt-auto">
            ...
            <p className="text-[11px] font-black tracking-[0.6em] text-slate-600 uppercase mb-2">프리미엄 전략 보고서</p>
            <p className="text-[10px] text-slate-800 tracking-tight italic">본 보고서는 AI 분석 모델과 명리학적 데이터를 기반으로 작성되었습니다.</p>
          </div>
        </div>
      </PageWrapper>
