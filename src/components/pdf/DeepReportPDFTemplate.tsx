...
const clean = (text: string | undefined): string => {
  if (!text) return '';
  return text.replace(/\(1,000자 이상\)\s*/g, '').trim();
};
          ...
          return (
            <div key={i}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className={`text-[13px] font-black tracking-tight ${lightTheme ? 'text-slate-900' : 'text-white'}`}>{title}</p>
              </div>
              {body && <p className={`text-[12.5px] leading-[1.85] pl-3 ${lightTheme ? 'text-slate-600' : 'text-slate-300'}`}>{body}</p>}
            </div>
          );
        }
        return <p key={i} className={`text-[12.5px] leading-[1.85] ${lightTheme ? 'text-slate-600' : 'text-slate-300'}`}>{para}</p>;
      })}
    </div>
  );
};
  ...
  >
    ...
    {pageNumber && (
      <div className={`absolute bottom-[8mm] left-0 w-full px-[22mm] flex items-center gap-3 ${bg === 'bg-slate-950' ? 'text-slate-600' : 'text-slate-300'}`}>
        ...
        <span className="text-[9px] font-black tracking-[0.3em] opacity-60 uppercase">Page {pageNumber} of {totalPages}</span>
        <div className="flex-1 h-px bg-current opacity-20" />
      </div>
    )}
  </div>
  ...
  const GaugeBar = ({ el }: { el: string }) => {
    ...
    return (
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <div className="w-8 text-right text-[10px] font-black text-slate-800">{val}%</div>
      </div>
    );
  };
      ...
      <PageWrapper pageNumber={2} totalPages={TOTAL}>
        ...
        <div className="grid grid-cols-12 gap-4 mb-6">
          ...
          <div className="col-span-4 flex flex-col gap-4">
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 flex-1">
              <p className="text-[10px] font-black text-slate-400 mb-3 uppercase tracking-tighter">Energy Balance</p>
        ...
        <div className="bg-amber-50/50 border border-amber-200 rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-sm font-black text-amber-900 tracking-tight italic">Your Personal Lucky Totems</p>
          </div>
          ...
          {(sajuData?.quarterlyLuck || []).map((q: any, i: number) => (
            <div key={i} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[8px] font-black text-slate-400 mb-2 uppercase tracking-widest">Master Key</p>
                <p className="text-xs font-bold text-slate-700 leading-relaxed italic">" {q.point} "</p>
              </div>
            </div>
          ))}
        </div>
