  ...
  useEffect(() => {
    if (isOpen) {
      const setAllAvailable = () => {
        ...
        setFormData(prev => ({ ...prev, reservationDate: prev.reservationDate || availableDates[0] || "" }));
      };
      setAllAvailable();
    }
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50 shrink-0">
           <div className="flex justify-center mb-4">
             <span className="px-3 py-1 bg-amber-50 text-amber-600 text-xs font-black rounded-full border border-amber-200 animate-pulse flex items-center gap-1">
               <Clock className="w-3 h-3" /> 실시간 분석 진행 중! (주문 순차 제작)
             </span>
           </div>
           <button 
           ...
           >
             ...
             {amount.toLocaleString()}원 결제하고 리포트 신청하기
           </button>
