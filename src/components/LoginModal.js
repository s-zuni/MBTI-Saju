import React from 'react';
import { X } from 'lucide-react';

const LoginModal = ({ handleLogin, openSignupModal, closeAllModals }) => {
  return (
    <div id="loginFormView" className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-gray-900">로그인</h3>
        <button onClick={closeAllModals} className="p-2 hover:bg-gray-100 rounded-full">
          <X className="w-5 h-5 text-gray-500" />
        </button>
      </div>
      <form onSubmit={handleLogin} className="space-y-4">
        <input
          type="email"
          id="loginEmail"
          required
          placeholder="이메일"
          className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 outline-none"
        />
        <input
          type="password"
          id="loginPw"
          required
          placeholder="비밀번호"
          className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 outline-none"
        />

        <div id="loginError" className="hidden text-red-500 text-sm text-center"></div>

        <button
          type="submit"
          id="loginBtn"
          className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all"
        >
          로그인
        </button>
        <button
          type="button"
          onClick={() => {
            // handleGoogleLogin();
          }}
          className="w-full py-3 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-bold rounded-xl flex items-center justify-center gap-2"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Google로 로그인
        </button>
        <p className="text-center text-xs text-gray-400">
          아직 계정이 없으신가요?{' '}
          <button
            type="button"
            onClick={openSignupModal}
            className="text-indigo-600 font-bold underline"
          >
            무료 분석 시작
          </button>
        </p>
      </form>
    </div>
  );
};

export default LoginModal;