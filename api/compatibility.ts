    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            (3) ${isPartnerMode ? '상대방과 나의 MBTI & 사주 심층 궁합' : '종합 이상 궁합 분석 및 관계 조언'} (약 500자)
               - ${isPartnerMode ? '핵심 시너지 요소와 그 이유' : '나와 가장 잘 맞는 유형의 특징과 그 이유'}
               - 피해야 할 행동 및 관계 유지 시 주의점 (상대방이 없는 경우 본인의 연애/인간관계 습관 교정 제언)

            분석 데이터 필드:
            1. score: 0~100 사이의 숫자로 종합 점수를 산출하세요. (상대방이 없어도 '이상 궁합 부합도'를 기준으로 생성)
            2. summary: 핵심을 찌르는 한 줄 요약을 작성하세요.
            3. keywords: 특징을 나타내는 키워드 3개를 배열로 제공하세요.
            4. details:
               - ideal_mbti: 위 (1)번 내용을 작성하세요.
               - ideal_saju: 위 (2)번 내용을 작성하세요.
               - overall_compatibility: 위 (3)번 내용을 작성하세요. (시너지 및 주의점 포함)
