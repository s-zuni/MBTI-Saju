...
const fortuneItemSchema = z.object({
    fortune: z.string().describe("매우 상세하고 디테일한 운세 분석 내용 (분량을 풍부하게 작성하고, 최소 3~4개의 구체적인 단락으로 구성하여 가독성을 높이기 위해 줄바꿈(\\n\\n)을 적극 사용할 것)"),
    lucky: luckySchema,
...
4. 각 궁에 자리한 주성/부성(stars)을 1-3개 명시하고, 해당 궁의 해설(analysis)을 작성할 때 다음 규칙을 철저히 지키세요:
   - 규칙 A (궁의 개념 설명 금지): 해당 궁 자체의 학계 정의나 개념(예: "명궁은 나의 성격과 인생을 뜻하며...", "형제궁은 친구와 동료 관계를 의미하고...")은 절대로 작성하지 마세요. 사용자는 자미두수 이론이 아니라 자신에게 닥칠 미래와 운명을 알고 싶어 합니다.
   - 규칙 B (사용자 맞춤 성격 및 운명 해석): 사용자의 사주 정보와 해당 궁에 위치한 별들의 조합을 기반으로 사용자의 구체적인 성격과 미래의 흐름을 직접적으로 설명하세요.
   - 규칙 C (행동 조언 및 주의사항 필수): 해당 궁의 특징에 맞추어 앞으로 어떤 것들을 특히 유의하고 조심해야 하는지 구체적으로 서술하세요.
   - 문장 예시 스타일:
     * 명궁: "[명궁]에 [태양성]과 [대음성]이 들어와 있어 [사람들에게 강한 호감을 주고 뚜렷한 목표를 향해 활기차게 나아가는] 성격입니다. 하지만 [의욕이 앞서 본인의 한계를 넘어서는 무리한 약속을 하기 쉬우니, 남을 도울 때 자신의 에너지를 먼저 챙기도록] 조심해야 합니다."
